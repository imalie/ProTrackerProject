...
{
    global $detail_id, $id, $proOwnerId, $proName, $address, $approxBudget, $startDate, $endDate, $planDoc, $releaseDate, $releaseUser, $boqDocArray,$status,$isStages;
    //define db query
    ...
    $isStages = false;
    $dbQueryIsStage = "SELECT is_stages FROM project WHERE id = '".$detail_id."';";
    if ($dbResultIsStage = mysqli_query($conn, $dbQueryIsStage)){
        if (mysqli_num_rows($dbResultIsStage) > 0) {
            $rowIsStage = mysqli_fetch_assoc($dbResultIsStage);
            if ($rowIsStage['is_stages'] == '0'){
                $isStages = false;
            } else {
                $isStages = true;
            }
            ...
            <div class="content">
                <div class="container-fluid">
                   ...
                   <div class="row" style="padding-left: 20px">
                        ...
                        if ($isStages == true) {
                            echo '
                            <a class="btn btn-success" target="_blank" href="progress.php?id='.$detail_id.'">Go to Progress</a>
                            ';
                        }
                        ?>
                    </div>
